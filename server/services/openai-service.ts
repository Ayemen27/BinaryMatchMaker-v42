    ...
    try {
      ...
      try {
        ...
        if (errorMessage.includes("exceeded your current quota") || errorObj?.code === 'insufficient_quota') {
          ...
          throw new Error(
            "تم تجاوز حد الاستخدام المسموح لخدمة الذكاء الاصطناعي. " +
            "يمكنك اختيار إحدى الحلول التالية:\n" +
          ...
          // خطأ في مفتاح API
          throw new Error(
            "مفتاح API الخاص بالذكاء الاصطناعي غير صالح. " +
            "يرجى التحقق من التالي:\n" +
            "1. إذا كنت تستخدم مفتاح API خاص بك، تأكد من صحته وتحديثه من صفحة الإعدادات.\n" +
            "2. يمكنك استخدام الخوارزميات التقليدية مؤقتًا بتحويل زر الذكاء الاصطناعي إلى وضع الإيقاف.\n" +
          ...
          throw new Error(
            "تم تجاوز الحد الأقصى لعدد طلبات الذكاء الاصطناعي في وقت قصير. " +
            "هذا يحدث عند إرسال الكثير من الطلبات خلال فترة زمنية قصيرة. " +
            "يمكنك:\n" +
            "1. الانتظار لبضع دقائق ثم المحاولة مرة أخرى.\n" +
            "2. استخدام الخوارزميات التقليدية مؤقتًا.\n" +
            "3. ترقية اشتراكك للحصول على حد أعلى للاستخدام."
          );
        } else if (errorMessage.includes("server error") || errorObj?.code?.includes('server')) {
          // خطأ في الخادم
          throw new Error(
            "واجهت خدمة الذكاء الاصطناعي مشكلة فنية. " +
            "هذا خطأ مؤقت من جانب الخادم وليس من جانبك. " +
            "يمكنك:\n" +
            "1. الانتظار لبضع دقائق والمحاولة مرة أخرى.\n" +
            "2. استخدام الخوارزميات التقليدية مؤقتًا.\n" +
            "3. التحقق من حالة خدمة الذكاء الاصطناعي من خلال الموقع الرسمي."
          );
        } else if (errorMessage.includes("model") || errorObj?.code?.includes('model')) {
          // خطأ في النموذج
          throw new Error(
            "النموذج المطلوب من خدمة الذكاء الاصطناعي غير متاح حاليًا. " +
            "قد يكون النموذج محدودًا أو قيد الصيانة. " +
            "يمكنك:\n" +
            "1. تجربة استخدام الخوارزميات التقليدية مؤقتًا.\n" +
            "2. المحاولة مرة أخرى لاحقًا عندما يعود النموذج للعمل."
          );
        } else if (errorMessage.includes("context length") || errorObj?.code?.includes('context_length')) {
          // خطأ في طول السياق
          throw new Error(
            "تجاوز طلب الذكاء الاصطناعي الحد الأقصى المسموح به للمحتوى. " +
            "يمكنك:\n" +
            "1. استخدام الخوارزميات التقليدية التي لا تخضع لهذا القيد.\n" +
            "2. تبسيط طلبك واستخدام معلمات أقل."
          );
        } else if (errorMessage.includes("content filter") || errorObj?.code?.includes('content_filter')) {
          // خطأ في فلترة المحتوى
          throw new Error(
            "تم رفض طلب الذكاء الاصطناعي بسبب قيود المحتوى. " +
            "يحتوي الطلب على محتوى يخالف سياسات الاستخدام. " +
            "يرجى استخدام الخوارزميات التقليدية بدلاً من ذلك."
          );
        }
        
        // إعادة إلقاء الخطأ الأصلي مع معلومات إضافية إذا كان خطأً آخر
        throw new Error(
          "تعذر استخدام خدمة الذكاء الاصطناعي لسبب غير متوقع. " +
          "يمكنك تجربة:\n" +
          "1. استخدام الخوارزميات التقليدية بتحويل زر الذكاء الاصطناعي إلى وضع الإيقاف.\n" +
          "2. المحاولة مرة أخرى لاحقًا.\n" +
          "3. التواصل مع الدعم الفني إذا استمرت المشكلة."
        );
      }
    } catch (error) {
