...
function PaymentPopup({ 
  ...
  const [paymentMethod, setPaymentMethod] = useState<'stars' | 'credit'>('stars');
  ...
  return (
    <div className="popup-overlay fixed inset-0 z-50 bg-black/70 flex items-center justify-center">
      <div 
        ref={popupRef} 
        className="popup-container w-full max-w-md bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-xl mx-4"
      >
        <div className="popup-header bg-gradient-to-r from-amber-400 to-yellow-500 p-4 relative">
          <button 
            onClick={onClose} 
            className="absolute left-2 top-2 text-white hover:bg-black/20 rounded-full p-1"
            ...
            <div className="grid grid-cols-1 gap-2">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">{t('telegramStars')}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('instantActivation')}</p>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('securePayment')}</p>
                  </div>
                </div>
                <div className="font-bold">${plan.priceInUSD}</div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-3">
              <div className="flex justify-between py-1">
                ...
                <span className="font-medium">
                  {telegramUser?.username ? `@${telegramUser.username}` : telegramUser?.id || t('guest')}
                </span>
              </div>
              ...
              <div className="flex justify-between py-1">
                <span className="text-gray-500 dark:text-gray-400">{t('botVersion')}:</span>
                <span className="font-medium">{botVersion}</span>
              </div>
        ...
        <div className="popup-footer p-4 border-t border-gray-200 dark:border-gray-700">
          <Button 
          ...
          >
            {isProcessing ? (
              ...
              paymentMethod === 'stars' 
                ? `${t('confirmPaymentWithStars')} (${plan.price} ⭐)` 
                : `${t('confirmPaymentWithCard')} ($${plan.priceInUSD})`
            )}
          </Button>
          
          <p className="text-center text-xs text-gray-500 dark:text-gray-400 mt-2">
            {t('byProceedingAgree')} <a href="#" className="text-amber-500">{t('termsOfService')}</a>
          </p>
        </div>
      </div>
    </div>
...
export default function TelegramStarsMiniApp() {
  ...
  // معالجة الدفع بعد تأكيد النافذة المنبثقة
